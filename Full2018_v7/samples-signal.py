...
files=[]
if useDYtt:
  files = nanoGetSampleFiles(mcDirectory, 'DYJetsToTT_MuEle_M-50') + nanoGetSampleFiles(mcDirectory, 'DYJetsToLL_M-10to50-LO_ext1')

else:
  files = nanoGetSampleFiles(mcDirectory, 'DYJetsToLL_M-50') + nanoGetSampleFiles(mcDirectory, 'DYJetsToLL_M-10to50-LO_ext1')
...
samples_temp = samples.copy()
for key, value in samples_temp.items():
    if isinstance(value["name"][0][1], list) and len(value["name"][0][1]) == 0:
        del samples[key]
